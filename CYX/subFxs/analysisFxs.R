...
kmsc <- function(thisTrialData, tMax, plotKMSC=FALSE, grid) {
  library(survival)
  # ensure timeWaited = scheduledWait on rewarded trials
  thisTrialData$timeWaited[thisTrialData$trialEarnings != smallReward] = thisTrialData$scheduledWait[thisTrialData$trialEarnings != smallReward]
  
  # fit a kaplan-meier survival curve
  kmfit = survfit(Surv(thisTrialData$timeWaited, (thisTrialData$trialEarnings == smallReward), type='right') ~ 1, 
  ...
  for (gIdx in 1:length(grid)) {
    ...
    # it is also ok to use min(which(kmT>=g)) which generates qualitatively identitial results 
    # we use max(which(kmT<=g)) here for aesthetic concerns
...
wtwTS <- function(thisTrialData, tGrid, wtwCeiling, plotWTW = F) {
  ...
  quitIdx = thisTrialData$trialEarnings == smallReward
...
resample = function(ys, xs, Xs){
  isBreak = F
  # for better alignment (e.g., to ensure 5.0 == 5.0), round up
  xs = round(xs, 1)
  Xs = round(Xs, 1)
  # initialize Ys
