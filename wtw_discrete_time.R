...
H$group <- c(rep('Controls',length(pwC$Hazard)), rep('Depressed',length(pwD$Hazard)),rep('Ideators',length(pwI$Hazard)),rep('Attempters',length(pwA$Hazard)))
H$group <- as.factor(H$group)
H$group = factor(H$group,levels(H$group)[c(2:4,1)])

H$h <- as.numeric(H$h)
H$logh <- log(H$h)
H$times <- as.numeric(H$times)
H$n <- c(pwC$At.Risk,pwD$At.Risk,pwI$At.Risk,pwA$At.Risk)
# pdf("piecewise_hazard_fx_all_groups.pdf", width = 12, height = 4)
p1 <- ggplot(H,aes(times, h, color = group, alpha = n)) + geom_line(size = 1.5)+ 
